...
def max_weight_matching(G: ig.Graph, maxcardinality=False):
    """Adaptation of NetworkX algorithm for igraph"""
    class NoNode:
    ...
    gnodes = [v.index for v in G.vs]
    if not gnodes:
        return dict()  # don't bother with empty graphs
        ...
        for bv in path:
            if isinstance(bv, Blossom):
                ...
                nblist = [(bv, w)
                          ...
                          for w in G.neighbors(bv, ig.OUT)
                          if bv != w]
            for k in nblist:
                (i, j) = k
                if inblossom[j] == b:
...
def main():
    n = 200
    m = int(2e2)
    g = ig.Graph.Erdos_Renyi(n, m=m, directed=False, loops=False)
    g.es["weight"] = [1]*g.ecount()

    result = max_weight_matching(g)
    print(result)
